---
title: 'Fidelity: Stop-Loss Orders'
id: fidelity-stop-loss-orders
tags:
- stop-loss
created: '2026-06-17T15:11:38.266285Z'
updated: '2026-06-17T20:28:23.270989Z'
source: https://www.fidelity.com/learning-center/trading-investing/trading/stop-loss-orders
source_domain: www.fidelity.com
fetched_at: '2026-06-17T15:11:38.266119Z'
fetch_provider: builtin
status: evergreen
type: note
tier: unknown
content_type: unknown
deprecated: false
summary: Fidelity practitioner primer on stop-loss order mechanics (stop vs stop-limit,
  market-gap risk). Thin reference note.
---

Fidelity Learn | Financial articles, webinars, and more | Fidelity
Learn
Search Learn
Search Banner
Filter
{{contentType.contentType}}
,
{{contentType.count}}
items
Show all
See all
Back to top
Strategies for uncertain times
Market insights and investing ideas for today's markets.
2026 midyear outlook
Join Fidelity leaders as they share insights on the economic factors they’re watching for the second half of the year.
Options trading entails significant risk and is not appropriate for all investors. Certain complex options strategies carry additional risk. Before trading options, please read
Characteristics and Risks of Standardized Options
.
Supporting documentation for any claims, if applicable, will be furnished upon request.
Investing involves risk, including risk of loss.
Fidelity Brokerage Services LLC, Member NYSE,
SIPC
, 900 Salem Street, Smithfield, RI 02917
947793.9.16