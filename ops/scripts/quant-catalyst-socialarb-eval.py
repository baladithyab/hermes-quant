"""Phase 0: run the labeled social-arb cases through the REAL catalyst.eval harness.
...
against the 5 documented Camillo cases with REAL forward returns from yfinance.

This is a measurement, not a production wire-up: the graph/lexicon extensions are
passed explicitly to the harness (graph=, aliases=) so nothing live is mutated. If
the edge passes, THEN we promote to the seed YAML + classify.py.
"""
...
def main():
    ...
    print(f"    cases={prec.n_cases} scored={prec.n_scored} hits={prec.hits} hit_rate={prec.hit_rate} (min 0.60)")
