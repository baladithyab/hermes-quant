...
def auto_approve_actionables(actionables: list[dict]) -> list[dict]:
    ...
    if caps_enabled:
        try:
            ...
            #
            # CORRECTNESS FIX (2026-06-10): seed from the CANONICAL paper-book
            # projection — reconstruct_portfolio_state(reactor_filter="paper") —
            # NOT from state.db.positions.quantity. The state.db positions table is
            # written by the ADDITIVE state/portfolio_state.py::reconstruct_from
            # projection across ALL reactors, where `quantity` accumulates raw
            # SHARE COUNTS, not normalized signed target weights. Treating those
            # share counts as target_position_pct made the cap read a phantom
            # gross of ~402% (e.g. a corrupt AAPL row at qty=399.93 from the
            # 2026-06-08 reconstruct_from-after-flatten incident) and silenced
            # EVERY advisor auto-fire on fake over-leverage while the real paper
            # book was BA -0.20 = 20% gross. reconstruct_portfolio_state uses the
            # same latest-target-supersedes, paper-only semantics as the ADR-0087
            # cap and the autonomous tick, so the advisor cap now agrees with every
            # other firing surface. See docs/architecture/
            ...
            # reconstruct_portfolio_state is fail-SOFT — a missing, mis-mounted,
            # or unreadable executions.jsonl returns an EMPTY PortfolioState
            # (cash=100%) instead of raising. For a SAFETY gate that is wrong: a
            # bus we cannot read is "headroom UNKNOWN", not "book is flat". Left
            # unguarded, an operator with real paper exposure + a bad/mis-mounted
            # bus would size against a fabricated 100%-cash book and auto-fire
            # uncapped — the exact fail-OPEN posture PR #82 closed. So probe the
            # bus explicitly and RAISE on missing/unreadable, letting the except
            # block below fail CLOSED (block every fire). A bus that EXISTS and
            # reads — even if it parses to an empty book (genuinely no open paper
            # positions) — is accepted and seeds a flat running book. A system
            # that has never recorded a paper fill blocking its advisor auto-fire
            # is the safe default; the autonomous tick / any prior fill creates
            # the bus.
