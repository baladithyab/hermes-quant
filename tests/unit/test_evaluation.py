...
def test_lookahead_random_score_passes():
    ...
    result = shuffle_timestamps_test(
        random_scorer, df, n_shuffles=10, seed=1,
    )
...
def test_lookahead_constant_score_passes():
    ...
    df = _hourly_bars(100)
    result = shuffle_timestamps_test(
        lambda bars: 1.0, df, n_shuffles=10,
    )
...
def test_lookahead_real_signal_passes():
    ...
    result = shuffle_timestamps_test(
        momentum_scorer, df, n_shuffles=10,
    )
...
def test_lookahead_result_str():
    df = _hourly_bars(100)
    result = shuffle_timestamps_test(
        lambda bars: 0.5, df, n_shuffles=5,
    )
    s = str(result)
...
def test_dsr_rejects_tiny_n_observations():
    with pytest.raises(ValueError, match=">= 30"):
        deflated_sharpe(
            observed_sharpe=1.0, n_trials=1, n_observations=10,
        )
...
def test_dsr_rejects_invalid_n_trials():
    with pytest.raises(ValueError, match="n_trials"):
        deflated_sharpe(
            observed_sharpe=1.0, n_trials=0, n_observations=100,
        )
