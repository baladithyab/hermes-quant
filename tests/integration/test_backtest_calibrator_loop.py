...
def test_replay_creates_long_lived_aggregator_when_learn_enabled():
    ...
    r = replay(
        bars, symbol="TEST", asset_class="equity", timeframe="1h",
        warmup_bars=60, advisor_recommend=advisor,
    )
...
def test_settlement_count_matches_decisions_within_window():
    ...
    r = replay(
        bars, symbol="TEST", asset_class="equity", timeframe="1h",
        warmup_bars=60, settlement_horizon_bars=1,
...
def test_disabling_learn_zeroes_settlements():
    ...
    r = replay(
        bars, symbol="TEST", asset_class="equity", timeframe="1h",
        warmup_bars=60, learn_from_fills=False,
        advisor_recommend=advisor,
    )
...
def test_posteriors_evolve_for_correct_analyst():
    ...
    assert a_stats["n_observations"] > 50, \
        "expected most decisions to settle"
...
def test_settlement_tail_bound():
    ...
    r = replay(
        bars, symbol="TEST", asset_class="equity", timeframe="1h",
        warmup_bars=60, settlement_horizon_bars=horizon,
        advisor_recommend=advisor,
    )
    # n_decisions = active_bars; settlements drop the last `horizon` of them
    active_bars = n_bars - 60
    expected_max_settlements = active_bars - horizon + 1  # +1 because last horizon scheduled may still settle on final pass
...
def test_settlement_horizon_zero_treated_safely():
    ...
    r = replay(
        bars, symbol="TEST", asset_class="equity", timeframe="1h",
        warmup_bars=60, settlement_horizon_bars=0,
        advisor_recommend=advisor,
    )
...
def test_to_markdown_report_includes_posterior_table():
    ...
    bars = _bars(200)
    advisor = _make_fake_advisor(lambda i: 1 if i % 3 == 0 else 0,
                                  analysts=("alpha_voice", "beta_voice"))
    r = replay(
        bars, symbol="TEST", asset_class="equity", timeframe="1h",
        warmup_bars=60, advisor_recommend=advisor,
    )
...
def test_config_hash_reflects_learn_flag():
    ...
    r_with = replay(bars, symbol="TEST", asset_class="equity", timeframe="1h",
                    warmup_bars=60, learn_from_fills=True,
                    advisor_recommend=advisor1)
    r_without = replay(bars, symbol="TEST", asset_class="equity", timeframe="1h",
                       warmup_bars=60, learn_from_fills=False,
...
def test_aggregator_update_exception_is_isolated():
    ...
    r = replay(
        bars, symbol="TEST", asset_class="equity", timeframe="1h",
        warmup_bars=60, advisor_recommend=advisor, aggregator=agg,
    )
