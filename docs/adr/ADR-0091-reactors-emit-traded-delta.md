...
- [x] Full `pytest` sweep green; firing/cap path (`reconstruct_portfolio_state` seed) unchanged; **`executions.jsonl` byte-identical** before/after (checksum gate — proves no mutation). **(EVIDENCE 2026-06-16, corrected: the original acceptance only re-ran a 12-file subset and was over-claimed — a concurrent review (`wlmy48a6a`) caught 6 failures in `tests/unit/test_fire_equity_close_cs21.py`, a branch regression where cs36 added a mandatory `decision_price` kwarg the cs21 test never adopted, NOT a normalizer/Option-E defect. Fixed (the 5 cs21 call sites now pass `decision_price=200.0`); the REAL full sweep `pytest tests/unit tests/ops tests/state` = 2623 passed, 2 skipped, 0 failed. The byte-identical checksum sub-claim is backed by `tests/state/test_reconcile_derived_net_and_log_checksum.py`.)**
- [ ] **Operator-gated live heal** (human call, see resolution banner): enable flag → `quant-ledger-reconcile --apply` (rebuild state.db from the UNCHANGED log) → AAPL=33.33sh/5%, BA=−0.20, seven paper symbols at single target; EOD snapshot clean; log checksum unchanged.
  - **⚠️ KILL-SWITCH COUPLING (seed `rt01`, review-team `w4fysiwv8`):** flipping `HERMES_QUANT_DELTA_NORMALIZER` ON also changes the LIVE kill-switch P&L basis — `autonomous.compute_cumulative_realized_pnl_pct` (the ADR-0016 §D9 trip basis, `autonomous.py:296→445`) calls `join_exit_fills`, whose i0c pre-pass is gated on the same flag, so the realized round-trips (and the fraction the kill-switch trips on) change. Before the live flip, run `tests/unit/test_kill_switch_pnl_basis_normalizer_flag.py` and review the flag-OFF vs flag-ON basis delta on the real book; do NOT treat the flip as a state.db-only change.
  - **⚠️ PARTIAL-ADD AMBIGUITY (seed `rt02`):** under flag-ON a genuine multi-tick partial-add toward the SAME absolute target is indistinguishable from a re-affirmation and collapses to one position. Before the flip, confirm no production lane emits two genuine partial adds carrying the same `target_position_pct`.
